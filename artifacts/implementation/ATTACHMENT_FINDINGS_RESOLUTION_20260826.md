...
| safe_process basename bypass | A path-qualified executable outside the trusted whitelist was accepted because only its basename was allowlisted | Path-qualified executables now require exact realpath membership in the whitelist; basename alone is insufficient | Runtime tests block a path-qualified malicious executable and another untrusted path; bare legacy tool names remain an explicit compatibility surface |
